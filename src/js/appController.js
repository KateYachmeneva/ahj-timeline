...
export default class AppController {
  constructor() {
    ...
    this.coordsValidator = new GeolocValidator();
  }
  ...
  openModal() {
    ...
    addForm.addEventListener('submit', (submitEv) => {
  ...
  showCoordsField() {
    ...
    document.getElementById('post-text').insertAdjacentHTML('afterend', '<span class="warning">Пожалуйста, введите координаты:</span>');
    document.getElementById('save-button').insertAdjacentElement('beforebegin', this.postLocation);
  }

  init() {
    this.showAddButton();
    if (this.posts) this.posts.forEach((post) => post.drawPost(post));
  }
}
